...
public class NetworkServer {
    ...
    private void checkAndCreateUserFolder() throws IOException {
        System.out.println("[checkAndCreateUserFolder] clientLocalPath:"+Property.getClientsRootPath()+"/"+cnt_clients);
        System.out.println("[checkAndCreateUserFolder] clientServerPath:"+Property.getServerRootPath()+"/"+cnt_clients);

        //Реализация на IO
        File file = new File(Property.getClientsRootPath()+"/"+cnt_clients);
        if (!file.exists()) FileUtility.createDirectory(file.getPath());
        file = new File(Property.getServerRootPath()+"/"+cnt_clients);
        if (!file.exists()) FileUtility.createDirectory(file.getPath());
    ...
    private void  createClientHandler(Socket clientSocket) {
        ...
        clientHandler.setClientId( cnt_clients );
    ...
    public void sendMessage(int receiver, Command commandMessage) throws IOException {
        System.out.println("[NetworkServer] count ClientHandler: "+clients.size());
        for (ClientHandler client : clients) {

            if (client.getClientId()== receiver) {
                client.sendMessage(commandMessage);
                System.out.println("[NetworkServer] сообщение отправлено клиенту с id "+receiver);
